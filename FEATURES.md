...
_Regenerated 2026-07-09 — 11 live · 13 building · 22 planned_
...
| ✅ LIVE | **codeio serve (web IDE + app server)** | P1/P5/P7 | `services/crates/codeio-cli/src/serve.rs` | Std-only HTTP web IDE (Termux-buildable): 3 tabs — Editor (Run), Blueprint (live IR graph canvas with pan/pinch-zoom, colored by node kind), System (real live/building/planned status). The GUI window into the whole system. Binds 0.0.0.0 for phone/LAN. |
...
| 🚧 BUILDING | **Codebase map (camera semantic-zoom over the whole repo)** | P5 | `services/crates/codeio-cli/src/serve.rs` | Unified blueprint surface (blue drafting sheet, border, title block). One scale-through continuum SYSTEM to MODULE to SYMBOLS to SOURCE: zoom all the way in and actual file code renders on-sheet via /source. Non-passive pinch/pan/wheel. DRAW LOGIC overlays the algorithm as drafted symbols. |
...
| 📋 PLANNED | **Visual workbench (blueprint designer, semantic zoom)** | P5/P8 | `docs/workbench-vision.md` | Self-hosting blueprint tool; HTML/CSS/JS designer chrome; semantic zoom blueprint->UPL->lower-constructs->source as IR-tier projections; router index links zoom levels; trace log = live stack trace. |
...
| 📋 PLANNED | **Language service slots (Python/JS/Java/C++/PHP/Lisp/HTML/CSS/Haskell/Rust)** | P6/P8 | `docs/language-services.md` | Reserved adapter node slots for each host language, tree-sitter-backed (M8), filled DETECT->LIFT->ROUND_TRIP as we scale. Framework laid; adapters not yet built. |
